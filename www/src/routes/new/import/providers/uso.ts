...
const USo_SURGE_PATTERN = new URLPattern('https://uso-archive.surge.sh/');
const USo_KKX_PATTERN = new URLPattern('https://uso.kkx.one/style/:id');
...
export default {
  check: (url) =>
    url.startsWith('https://uso-archive.surge.sh/?style=') ||
    url.startsWith('https://uso.kkx.one/style/'),
  import: async (url) => {
    let id = getStyleId(url);

    const data = (await fetchAsJson(
      'https://raw.githubusercontent.com/uso-archive/data/flomaster/data/styles/' + id + '.json',
    )) as USoArchiveData | undefined;
    if (!data) throw new Error('Failed to fetch style from USo archive');
    const code = await fetchRawFile(
      'https://raw.githubusercontent.com/uso-archive/data/flomaster/data/usercss/' +
        id +
        '.user.css',
    );
