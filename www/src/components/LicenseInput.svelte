    ...
    el.addEventListener('focusout', () => {
      setTimeout(() => {
        if (el!.value.trim() && !Object.keys(licenses).includes(el!.value)) {
          warning = "Are you sure that license exists? I can't seem to find it in the SPDX database.";
        } else {
          warning = null;
        }
      }, 200);
...
<input type="text" list="licenses" {id} {required} bind:this={el} bind:value class="license-input" />
